...
\paragraph{Balance inconsistency.} Each transaction's origin-account balances were checked against the identity implied by its type: \(\texttt{oldbalanceOrg} + \texttt{amount} = \texttt{newbalanceOrig}\) for \texttt{CASH\_IN} (a deposit, which increases the origin balance), and \(\texttt{oldbalanceOrg} - \texttt{amount} = \texttt{newbalanceOrig}\) for every other type (\texttt{CASH\_OUT}, \texttt{DEBIT}, \texttt{PAYMENT}, \texttt{TRANSFER}, all of which withdraw from the origin balance), tolerance 0.01. An earlier version of this check applied the withdrawal formula uniformly, including to \texttt{CASH\_IN}, which mis-flagged deposits regardless of whether they were recorded correctly; this was corrected before finalizing the dataset. \textbf{Result: 3,953,846 transactions (62.14\%)} flagged --- still a proportion high enough to look like a serious defect. It is not, but the reason is more specific than ``destination balances are untracked'': this check never reads the destination-account columns at all. Verification on the real dataset shows 96.8\% of the flagged non-\texttt{CASH\_IN} rows have \(\texttt{amount} > \texttt{oldbalanceOrg}\) --- PaySim permits an origin account to be debited beyond its recorded balance and floors \texttt{newbalanceOrig} at 0 rather than rejecting the transaction or allowing a negative balance. This is a known characteristic of the simulator's origin-side accounting, not a data-entry error, and removing 62\% of the dataset to ``fix'' it would be the wrong response. Flagged via \texttt{is\_balance\_inconsistent} and kept.
...
Balance inconsistency & 6,362,620 & 3,953,846 (62.14\%) & Flagged, retained \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Implementation and verification.} All six checks are vectorized \texttt{pandas}/\texttt{numpy} functions --- no row-wise iteration over 6.36 million rows --- composed into a single orchestrator that runs the three removal checks first, then the three flag checks. Twenty-eight unit tests cover the individual checks, the direction-aware balance-inconsistency formula, the Tukey-fence fit/transform split, and the orchestrator; all twenty-eight pass. The before/after report is generated programmatically from the same code that performs the checks, so it cannot drift from what was actually executed. Running the pipeline against the full dataset reproduced the preliminary-survey figures exactly. The resulting 27-column dataset was written out as the final artifact, along with a smaller stratified sample (same 0.1291\% fraud rate) for quick inspection.
