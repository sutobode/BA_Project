...
| balance_inconsistent | 6362620 | 3953846 | flagged (kept) | Column: is_balance_inconsistent. Direction-aware check on the ORIGIN account only (oldbalanceOrg/newbalanceOrig vs amount and type - CASH_IN deposits, other types withdraw); does not read destination/merchant balances at all. **Most flags on the real dataset come from PaySim recording amount > oldbalanceOrg (an over-draft/insufficient-funds transaction) with newbalanceOrig floored at 0 rather than rejected or negative - a known PaySim data characteristic, not a data-entry error** - do not interpret a high count here as a data quality problem. |
