...
- `assign_train_test_split()` (70/30, seed=123) chỉ phục vụ việc fit `amount_percentile_reference` không leak — không phải split chính thức cho Model Development; Module 5 nên tự quyết định chiến lược split của mình (có thể cần stratified hoặc time-based, khác với utility này).
