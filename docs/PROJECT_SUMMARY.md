...
### 3.2. 12 field đã sinh

`hour_of_day`, `is_night_transaction`, `customer_account_age_days`, `device_id`, `browser`, `device_type`, `new_device_flag`, `billing_country`, `ip_country`, `ip_billing_distance_km`, `shipping_billing_mismatch`, `failed_payment_attempts_24h`.

Nguyên tắc: chỉ field có lý do hành vi thật mới điều kiện theo `isFraud` (giới hạn hệ số 2–4 lần baseline); field không có cơ sở hành vi sinh độc lập. Chi tiết công thức từng field: xem README mục 7 hoặc [`docs/DATA_DICTIONARY.md`](DATA_DICTIONARY.md).
...
Row count và tỷ lệ fraud giữ nguyên (0,1291%). Leakage check: **12/12 field PASS** (ngưỡng AUC < 0.75, Cramér's V < 0.5). Chi tiết số đo: xem README mục 9.
...
- **70/70 unit test pass** (52 cho Synthetic Generation + 18 cho Data Cleaning)
...
```
transactions_synthetic.parquet / .csv     — sau Giai đoạn 1 (23 cột)
transactions_synthetic_sample.csv         — mẫu ~5.000 dòng
transactions_cleaned.parquet / .csv       — sau Giai đoạn 2, BẢN CUỐI CÙNG (26 cột)
