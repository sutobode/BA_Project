...
\begin{itemize}
    ...
    \vspace{0.2cm}
    \item \textbf{Balance inconsistency} (origin account, direction-aware by type): \textbf{3,953,846} transactions (62.14\%)\\
    \small Mostly PaySim allowing an over-draft on the origin balance, floored at 0 --- \textbf{not a data error, and not a destination/merchant issue}.
\end{itemize}
\end{frame}
...
\vspace{0.4cm}
\begin{center}
24 columns \(\rightarrow\) \textbf{27 columns}. Row count and fraud rate (0.1291\%) unchanged.
\end{center}
\end{frame}
