...
|---|---|---|
| Test suite | **118/118 PASS** | `.venv/Scripts/python.exe -m pytest tests/ -v` |
...
```
is_amount_outlier: amount=181000 so với Q1/Q3 của train split → giả sử vượt upper_fence → True
is_zero_amount: amount=181000 ≠ 0 → False
is_balance_inconsistent: type=TRANSFER (rút tiền, dùng công thức trừ) → |181000 - 181000 - 0| = 0 ≤ 0.01 → False (khớp đúng công thức rút cạn tài khoản)
```
...
| **Balance inconsistency (origin account, direction-aware theo `type`)** | **3.953.846 dòng (62,14%)** | ~97% trong số này là PaySim ghi nhận `amount > oldbalanceOrg` (rút/chuyển quá số dư — over-draft) rồi "floor" `newbalanceOrig` về 0 thay vì cho số dư âm hoặc từ chối giao dịch — đặc điểm đã biết của PaySim, KHÔNG phải lỗi nhập liệu. Đây là account NGUỒN (`oldbalanceOrg`/`newbalanceOrig`), không liên quan đến account đích/merchant (mục 12b giải thích rõ vì sao diễn giải "merchant không track" trước đây là sai) |
...
**Vì sao chọn Flag thay vì xoá:** 16 dòng `amount=0` đều là fraud thật — xoá sẽ mất đúng 16 mẫu fraud hiếm; outlier `amount` lớn có thể là chính tín hiệu fraud mà model cần học; 62% "balance inconsistent" là đặc điểm nguồn dữ liệu (mục 12b), xoá sẽ mất phần lớn dataset một cách vô lý.
...
### 12b. Balance inconsistency — bug đã sửa (công thức sai hướng cho `CASH_IN`) và nguyên nhân thật của tỷ lệ cao

**Bug đã phát hiện và sửa:** `flag_balance_inconsistency` ban đầu dùng đúng 1 công thức `oldbalanceOrg - amount == newbalanceOrig` cho **mọi** loại giao dịch. Công thức này đúng cho `CASH_OUT`/`DEBIT`/`PAYMENT`/`TRANSFER` (tiền **rời khỏi** account nguồn), nhưng **sai hướng** cho `CASH_IN` — đây là giao dịch **nạp tiền vào** account nguồn, nên số dư phải **tăng** (`oldbalanceOrg + amount == newbalanceOrig`), không phải giảm. Dùng sai công thức cho `CASH_IN` khiến gần như 100% các dòng `CASH_IN` (1.399.284 dòng, 22% dataset) bị flag sai — kể cả những dòng ghi nhận số dư hoàn toàn đúng.

**Cách sửa:** `flag_balance_inconsistency(type_, old_balance_org, amount, new_balance_orig, tolerance=0.01)` giờ nhận thêm `type_`, dùng công thức cộng cho `CASH_IN` và công thức trừ cho 4 loại còn lại (xem docstring trong code). Test mới `test_flag_balance_inconsistency_does_not_flag_correctly_recorded_cash_in_that_would_fail_withdrawal_formula` xác nhận 1 giao dịch `CASH_IN` ghi đúng (`old=1000, amount=500, new=1500`) — vốn sẽ bị flag sai bởi công thức cũ — giờ không còn bị flag.

**Nguyên nhân thật của tỷ lệ cao (không phải "merchant không track"):** giải thích trước đây trong tài liệu — *"đặc điểm cố hữu của PaySim, nhiều giao dịch đến merchant/tài khoản đích không track số dư"* — **là diễn giải sai**, vì `flag_balance_inconsistency` chỉ đọc `oldbalanceOrg`/`newbalanceOrig` (account **nguồn**), không hề đọc `oldbalanceDest`/`newbalanceDest` (account **đích**/merchant) — 2 việc này độc lập với nhau. Verify thực tế trên data thật (sau khi sửa bug `CASH_IN`, chỉ xét 4 loại còn lại): **96,8%** số dòng bị flag là do PaySim cho phép `amount > oldbalanceOrg` (rút/chuyển tiền vượt quá số dư khả dụng — over-draft) rồi ghi `newbalanceOrig` là 0 (floor về 0) thay vì cho phép số dư âm hoặc từ chối giao dịch — đây **là** đặc điểm cố hữu của PaySim, nhưng nằm ở **account nguồn**, không phải account đích/merchant.

**Số liệu sau khi sửa:** tỷ lệ `is_balance_inconsistent` giảm từ 80,45% (5.118.892 dòng, công thức sai) xuống **62,14% (3.953.846 dòng)** trên toàn bộ 6.362.620 dòng thật. Vẫn là tỷ lệ cao — nhưng giờ số liệu và giải thích đã khớp đúng với cơ chế thật của công thức, không còn mâu thuẫn nội tại.
...
| 6 | Balance inconsistency: account nguồn không đổi đúng chiều `amount` theo `type` (sai lệch > 0.01) — xem mục 12b | Flag, giữ nguyên | `is_balance_inconsistent` |
...
- **`is_balance_inconsistent`** (3.953.846 dòng, 62,14%): đánh dấu giao dịch mà số dư account **nguồn** không đổi đúng chiều theo `amount`/`type` (`CASH_IN` cộng, 4 loại khác trừ — mục 12b). Tỷ lệ cao (62%) khiến dễ hiểu lầm là lỗi dữ liệu nghiêm trọng — **thực chất phần lớn (96,8% sau khi loại `CASH_IN`) là do PaySim cho phép rút/chuyển vượt số dư khả dụng rồi floor số dư về 0**, một đặc điểm cố hữu của PaySim ở account nguồn, không liên quan đến account đích/merchant. Ý nghĩa sử dụng: (a) **không nên báo cáo con số 62% này như một vấn đề chất lượng dữ liệu** khi trình bày; (b) cột flag vẫn có giá trị làm feature phụ (bản thân việc "giao dịch có vượt số dư khả dụng hay không" có thể tương quan với hành vi tài khoản); (c) dùng để lọc subset "balance nguồn nhất quán" nếu một phân tích cụ thể cần dữ liệu balance đáng tin cậy.
...
| `is_balance_inconsistent` | 6.362.620 | 3.953.846 (62,14%) | flagged (kept) |
...
| `newbalanceOrig` | float32 | ≥ 0 | Số dư tài khoản nguồn **sau** giao dịch. So với `oldbalanceOrg ± amount` (chiều cộng/trừ theo `type`, mục 12b) để phát hiện `is_balance_inconsistent` |
| `nameDest` | string | Bắt đầu bằng `C` (khách hàng) hoặc `M` (merchant) | Mã định danh tài khoản/đối tượng nhận giao dịch |
| `oldbalanceDest` | float32 | ≥ 0, thực tế đến ~356 triệu | Số dư tài khoản đích **trước** giao dịch. Thường = 0 với merchant (không track) — **không** phải nguồn gốc của `is_balance_inconsistent` (check đó chỉ đọc account nguồn, xem mục 12b) |
...
| `is_balance_inconsistent` | bool | {True, False} (3.953.846 dòng = True) | Số dư account nguồn không đổi đúng chiều `amount` theo `type` (mục 12b) — phần lớn (96,8%) là PaySim cho phép over-draft rồi floor về 0, **đặc điểm cố hữu của PaySim ở account nguồn, không phải lỗi dữ liệu và không liên quan account đích/merchant**; tỷ lệ cao (62,14%) là bình thường, không nên báo cáo như vấn đề chất lượng dữ liệu |
...
**Q: Vì sao 62,14% dòng bị flag `is_balance_inconsistent` mà không coi là lỗi nghiêm trọng?**
A: Check này chỉ đọc account **nguồn** (`oldbalanceOrg`/`newbalanceOrig`), không đọc account đích/merchant — nên không thể giải thích bằng "merchant không track số dư" (mục 12b). Verify thực tế: 96,8% số dòng bị flag (sau khi loại `CASH_IN`, loại có bug hướng công thức đã sửa) là do PaySim cho phép `amount > oldbalanceOrg` (over-draft) rồi floor `newbalanceOrig` về 0 — đặc điểm cố hữu của PaySim ở account nguồn, không phải lỗi nhập liệu. Xoá 62% dataset vì lý do này là vô lý và sẽ phá huỷ tính đại diện của dữ liệu.
